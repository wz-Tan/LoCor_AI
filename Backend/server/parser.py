...
class DocumentParser:
    ...
    async def initialise_data(
        ...
        try:
            ...
            # Parse bytes
            company_description = await DocumentParser._read_pdf_or_docx(desc_filename, desc_bytes)
            inventory_data      = await DocumentParser._read_csv_or_xlsx(inv_filename, inv_bytes)
            sales_data          = await DocumentParser._read_csv_or_xlsx(sales_filename, sales_bytes)
            balance_sheet_data  = await DocumentParser._read_csv_or_xlsx(bal_filename, bal_bytes)
    ...
    @staticmethod
    async def upload_df_to_chromadb(
    ...
    @staticmethod
    async def _read_csv_or_xlsx(filename: str, file_bytes: bytes) -> pd.DataFrame | None:
        ext = filename.split(".")[-1].lower()
        if ext == "csv":
