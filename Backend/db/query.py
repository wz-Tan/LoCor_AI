...
def QueryFunction(query: list[str], collection_name: str):
    ...
    filtered_results = {"ids": [], "documents": [], "distances": [], "metadatas": []}
    ...
    for i in range(len(all_results["distances"])):
        ...
        for j in range(len(all_results["distances"][i])):
            if all_results["distances"][i][j] < 1.8:
...
def main():
    ...
    try:
        ...
        print('Error: "results" not as expected.', e)
